...
$result=mysqli_query($con,$sql) or die("ERROR : " . mysqli_error($dbconnection));
...
if ($status=='Active'){ 
 ...
 $sql = "INSERT into userlogs (username,name,action)  values('$username','$name','Successfully loggedin')";
...
if ($status=='Dormant'){ 
 ...
 $_SESSION['username']=$username;
 $_SESSION['password']=$password;
  $sql = "INSERT into userlogs (username,name,action)  values('$username','$name','Logged in to first user page , Account Dormant')";
mysqli_query($con,$sql) or die("ERROR : " . mysqli_error($con));
header("location:main/firstuser.php");
}
if($status=='Suspended'){ 
 $_SESSION['id']=$id;
 $_SESSION['name']=$name;
 ...
 $sql = "INSERT into userlogs (username,name,action)  values('$username','$name','Failed login user under suspension')";

header("location:index.php?suspend");
}
}
else {
	 $sql = "INSERT into userlogs (username,name,action)  values('$username','N/A','Failed login')";
