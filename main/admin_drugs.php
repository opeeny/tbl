...
//$code=clean($drugcolumn);
//$string = "Th*()is 999 is <<>> a ~!@# sample st#$%ring.";
$code= preg_replace("/[^a-zA-Z0-9]/", "", $drugcolumn);
mysqli_query($dbconnection,"INSERT INTO option_dstdrugs1(code,name) 
VALUES('$code','$name')") or die("ERROR : " . mysqli_error($dbconnection));
mysqli_query($dbconnection,"ALTER TABLE results_dst1_hist ADD `$code` VARCHAR(255) AFTER method") or die("ERROR : " . mysqli_error($dbconnection));
mysqli_query($dbconnection,"ALTER TABLE results_dst1 ADD `$code` VARCHAR(255) AFTER method") or die("ERROR : " . mysqli_error($dbconnection));
}
...
mysqli_query($dbconnection,"ALTER TABLE results_dst2_hist ADD `$code` VARCHAR(255) AFTER method") or die("ERROR : " . mysqli_error($dbconnection));

mysqli_query($dbconnection,"ALTER TABLE results_dst2 ADD `$code` VARCHAR(255) AFTER method") or die("ERROR : " . mysqli_error($dbconnection));
}
...
$drugcolumn=str_replace(' ', '', strtolower($code));
...
mysqli_query($dbconnection,"ALTER TABLE results_dst1 CHANGE `$old_code` `$drugcolumn` VARCHAR(255)") or die("ERROR : " . mysqli_error($dbconnection));
mysqli_query($dbconnection,"ALTER TABLE results_dst1_hist CHANGE `$old_code` `$drugcolumn` VARCHAR(255)") or die("ERROR : " . mysqli_error($dbconnection));
...
<div class="form-horizontal">
    ...
    <div class="col-sm-4 "> 
		<input type="text"  readonly name="code" value="<?php echo $code; ?>" class="form-control" placeholder="" required/>
    </div>
	</div>
