...
$insert=mysqli_query($dbconnection,"insert into option_spectype(code,name,status)
...
mysqli_query($dbconnection,"DELETE FROM option_spectype WHERE id=$delete_userid") or mysqli_error($dbconnection);
...
mysqli_query($dbconnection,"UPDATE  option_spectype SET status='Deactive' WHERE id=$suspended_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
mysqli_query($dbconnection,"UPDATE  option_spectype SET status='Active' WHERE id=$active_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
$update=mysqli_query($dbconnection,"UPDATE option_spectype SET name='$spectype',code='$code',status='$status' 
where id='$id'");
...
function delete_warning(node){
 var x=confirm("Are you sure you want to delete the specimen type from the database?");
 
 if(x == false)  {return false;}
}
 
 function edit_warning(node){
 var x=confirm("Are you sure you want to close this Specimen type?");
 ...
 var x=confirm("Are you sure you want to activate this closed Specimen Type");
...
$sql="SELECT * FROM  option_spectype WHERE id='$edit_id'";
...
while ($option_spectype= mysqli_fetch_array($studycodes_check,MYSQLI_ASSOC)) {
	$code= $option_spectype['code'];
	$spectype= $option_spectype['name'];
	$status= $option_spectype['status'];
	$id= $option_spectype['id'];
}
	} ?>

          <div class="form_head">SPECIMEN TYPE REGISTRATION <?php echo $id; ?></div>
...
<div class="form_head">LIST OF SPECIMEN TYPES  </div>
...
include("../includes/dbconfig.php");  
 $query=mysqli_query($dbconnection,"select * from option_spectype  ORDER BY id desc"); 
  //$display="Displaying System Users";
  $anddisplay="";
  $num_rows=mysqli_num_rows($query);
  if($num_rows>0)
  {?>
	<button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal">New Specimen Type</button>
  <!-- Modal -->
  <div class="modal fade" id="myModal" role="dialog">
<?php  require_once 'new_spectype.php'; ?>
    </div>
...
<tr>
  ...
  ?>
   ...
   <input type='hidden' name='delete_userid' value='<?php echo $study ?>'>
