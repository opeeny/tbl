...
$insert=mysqli_query($dbconnection,"UPDATE transporters SET initials='$initials',name='$name',contact='$contact' where id='$id'");
...
mysqli_query($dbconnection,"DELETE FROM transporters WHERE id=$delete_userid") or mysqli_error($dbconnection);
...
mysqli_query($dbconnection,"UPDATE  transporters SET status='Suspended' WHERE id=$suspended_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
mysqli_query($dbconnection,"UPDATE  transporters SET status='Active' WHERE id=$active_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
function delete_warning(node){
 var x=confirm("Are you sure you want to delete the Transporter from the database?");
 
 if(x == false)  {return false;}
}
 
 function edit_warning(node){
 var x=confirm("Are you sure you want to suspend this Transporter?");
...
$sql="SELECT * FROM  transporters WHERE id='$edit_id'";
...
while ($user= mysqli_fetch_array($transporters_check,MYSQLI_ASSOC)) {
	...
	<div class="form_head">UPDATING TRANSPORTERS INFORMATION</div>
...
<div class="form_head">TRANSPORTERS </div>
...
<?php
	...
	?>
		<button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal">New Transporter</button>
  <!-- Modal -->
  <div class="modal fade" id="myModal" contact="dialog">
<?php  require_once 'new_transporter.php'; ?>
    </div>
	<?php
include("../includes/dbconfig.php");  
 $query=mysqli_query($dbconnection,"select * from transporters  ORDER BY id desc"); 
