...
$checkduplicate=mysqli_query($dbconnection,"select *  from visitinterval   where name='$techname' ");

if(mysqli_num_rows($checkduplicate) <1)
{
$insert=mysqli_query($dbconnection,"insert into visitinterval (name,status) values('$techname','Active')"); 
if ($insert){
	}
...
mysqli_query($dbconnection,"DELETE FROM visitinterval WHERE id=$delete_userid") or mysqli_error($dbconnection);
...
mysqli_query($dbconnection,"UPDATE  visitinterval SET status='Suspended' WHERE id=$suspended_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
mysqli_query($dbconnection,"UPDATE  visitinterval SET status='Active' WHERE id=$active_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
$update=mysqli_query($dbconnection,"UPDATE visitinterval SET name='$spectype',initial='$initial',status='$status' 
where id='$id'");
...
function delete_warning(node){
 var x=confirm("Are you sure you want to delete the this result interval option from the database?");
 
 if(x == false)  {return false;}
}
 
 function edit_warning(node){
 var x=confirm("Are you sure you want to Suspend this result interval option?");
 ...
 var x=confirm("Are you sure you want to activate this closed result interval option");
...
$sql="SELECT * FROM  visitinterval WHERE id='$edit_id'";
...
<div class="form_head">VISIT INTERVAL OPTIONS  </div>
...
include("../includes/dbconfig.php");  
 $query=mysqli_query($dbconnection,"select * from visitinterval  ORDER BY id desc"); 
...
<?php  require_once 'new_visitinterval.php'; ?>
    </div>
 <div class="table-responsive">
<table  border="1" class="table">
<tr>
    
	<th>Visit Interval Option</th>
