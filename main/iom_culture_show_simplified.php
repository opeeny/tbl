...
$select_liquidcultureid="SELECT * FROM results_identification where 
labno='$labno' and (media='MGIT' OR media='7H11S')";
...
if($cultureid=='MTBc' or $cultureid=='POS'){$cultureid='P';}
if($cultureid=='NTM' or $cultureid=='NEG'){$cultureid='N';}
}
else{$cultureid='';}

$select_solidculture="SELECT * FROM results_solidculture where labno='$labno' and media='7H11S'";
...
while ($solidculture= mysqli_fetch_array($solidcultures,MYSQLI_ASSOC)) {
	$solidculture_7h11s=$solidculture['result_sqt'];
}
...
if($liquidculture_resdtp!=='' AND !is_numeric($liquidculture_resdtp)){
...
while ($option = mysqli_fetch_array($options,MYSQLI_ASSOC)) {
	...
	if($liquidculture_resznc=='P' AND $cultureid=='P' ){
	$finalcul='MTBc isolated';	
	}
	else if($liquidculture_resznc=='P' AND $cultureid=='N' ){
	$finalcul='NTM isolated';	
	}
	else if($liquidculture_resznc=='NS' 
	AND $liquidculture_resbap=='C' AND $solidculture_7h11s=='C' ){
	$finalcul='Contaminated';	
	}
	else if($liquidculture_resznc=='NS' AND $liquidculture_resbap=='C' AND $solidculture_7h11s=='NG' ){
	$finalcul='MTBc not isolated';	
	}
	else if(($solidculture_7h11s=='NG' OR $solidculture_7h11s=='C') AND ($liquidculture_resdtp=='NG' OR $liquidculture_resdtp=='NA')){
	$finalcul='MTBc not isolated';	
	}
	else if($cultureid=='N'){
	$finalcul='NTM isolated';	
