...
$select_liquidcultureid="SELECT * FROM results_identification where labno='$labno' 
and media='MGIT'";
...
$liquidcultid_count=mysqli_num_rows($liquidcultids);
 ...
 if($liquidcultid_count>0){ 
	...
	if($cultureid=='MTBc' or $cultureid=='POS'){
		$cultureid='P';
	}
	if($cultureid=='NTM' or $cultureid=='NEG'){
		$cultureid='N';
	}
 }else{
	$cultureid=''; 
...
if($liquidcultures_count>0){ ?>
	...
	$solidculture_res=$solidculture['interpretation'];
if($solidculture_res='Positive For Nontuberculosis Mycobacteria And Contaminated'
or $solidculture_res='Positive For Nontuberculosis Mycobacteria'
or $solidculture_res='Positive For Non Tuberculosis Mycobacteria And Contaminated'
or $solidculture_res='Positive For M Tuberculosis Complex And Contaminated'
or $solidculture_res='Positive For M.tuberculosis And Contaminated'
or $solidculture_res='Positive For M.tuberculosis Complex And Contaminated'
or $solidculture_res='Positive For Nontuberculosis Mycobacteria And Contaminated'){
	...
	if($liquidculture_resdtp<1){
	$sql="SELECT * FROM resultsoptions_bap where options='$liquidculture_resdtp'";
			...
			while ($option = mysqli_fetch_array($options,MYSQLI_ASSOC)) {
				$liquidculture_resdtp = $option['code'];
	...
	$sql="SELECT * FROM resultsoptions_zn where options='$liquidculture_resznc'";
	$options=mysqli_query($dbconnection,$sql) or die("ERROR : " . mysqli_error($dbconnection));

	while ($option = mysqli_fetch_array($options,MYSQLI_ASSOC)) {
	$liquidculture_resznc = $option['code'];
			
			}
	$liquidculture_resbap=$liquidculture['result_bap'];
	
	//show result instead of code for bap
	$sql="SELECT * FROM resultsoptions_bap where options='$liquidculture_resbap'";
			$options=mysqli_query($dbconnection,$sql) or die("ERROR : " . mysqli_error($dbconnection));

			while ($option = mysqli_fetch_array($options,MYSQLI_ASSOC)) {
				$liquidculture_resbap = $option['code'];
	...
	if($liquidculture_resdtp>0 AND $liquidculture_resznc=='P'
	AND ($liquidculture_resbap='C' OR $liquidculture_resbap=='N') 
	AND $cultureid=='N' ){
	$finalcul='NT M isolated';	
	}else if ($liquidculture_resdtp>0 AND $liquidculture_resznc=='P'
	AND ($liquidculture_resbap='C' OR $liquidculture_resbap=='N') 
	AND $cultureid=='P'){
		$finalcul='MTB isolated';
	}
	else if ($liquidculture_resdtp=='NG' AND $liquidculture_resznc=='ND'
	AND ($liquidculture_resbap=='ND') 
	AND $cultureid=='' ){
		$finalcul='MTB not isolated';
	}else if($liquidculture_resdtp>0 AND $liquidculture_resznc=='NS'
	AND ($liquidculture_resbap='C') 
	AND $cultureid=='' ){
...
<td><?php if($colldate=='0000-00-00'){echo '';} else echo @date('d-M-y',strtotime($colldate));?></td>
	...
	//ldtp is $liquidculture_resdtp id $cultureid bpa $liquidculture_resbap zn $liquidculture_resznc
?>
