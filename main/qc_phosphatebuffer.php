...
$insert=mysqli_query($dbconnection,"UPDATE qc_phosphatebuffer SET prepdate='$prepdate', comment='$comment',
...
$insert=mysqli_query($dbconnection,"insert into qc_phosphatebuffer (lotno,expdate,qcpassfail,performedby,performeddate,
...
mysqli_query($dbconnection,"DELETE FROM qc_phosphatebuffer WHERE id=$delete_userid") or mysqli_error($dbconnection);
...
mysqli_query($dbconnection,"UPDATE  qc_phosphatebuffer SET status='Suspended' WHERE id=$suspended_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
mysqli_query($dbconnection,"UPDATE  qc_phosphatebuffer SET status='Active' WHERE id=$active_userid") or die("ERROR : " . mysqli_error($dbconnection));
...
$sql="SELECT * FROM  qc_phosphatebuffer WHERE id='$edit_id'";
...
while ($user= mysqli_fetch_array($qc_phosphatebuffer_check,MYSQLI_ASSOC)) {
	...
	<div class="form_head">UPDATING FO-MYC-P002-B:  PHOSPHATE BUFFER QC  </div>
...
<div class="form-group"> 
 <input type="hidden" name="id" value="<?php echo $id ?>" >
      <label  class="col-sm-2 control-label">Lot No. of phosphate buffer:</label>
      <div class="col-sm-4"> 
        <input type="text" class="form-control" placeholder="Lot No. of NaOH-Citrate Solution" value="<?php echo $lotno ?>" name="lotno" />
      </div>
	  <label  class="col-sm-2 control-label">Phosphate Buffer Exp Date</label>
     <div class="col-sm-4"> 
...
include("../includes/dbconfig.php");  
 $query=mysqli_query($dbconnection,"select * from qc_phosphatebuffer  ORDER BY id desc"); 
...
<?php require_once 'new_qc_phosphatebuffer.php'; ?>
    </div>
	<?php if($num_rows>0)
  {?>
...
<div class="form_head">FO-MYC-P002-B: PHOSPHATE BUFFER  QC FORM  </div>


 <div class="table-responsive">
<table  border="1" class="table">
<tr><th> Preparation & Autoclaved Date</th>
     <th> Lot No. of phosphate buffer</th>
  <th>Phosphate Buffer Exp Date</th>
  <th>Performed By</th>
