...
<?php
if(isset($_POST['saveresultsdst2'])){
include("../includes/dbconfig.php");

$row_id=mysqli_real_escape_string($dbconnection,$_POST['row_id']);
mysqli_query($dbconnection,"INSERT INTO results_dst2_hist SELECT 'NULL',h.* FROM results_dst2 h WHERE h.id=$row_id") or die("ERROR : " . mysqli_error($dbconnection));
...
$drugcolumn=preg_replace('#\_#', '.', $drugcolumn);
	mysqli_query($dbconnection,"UPDATE results_dst2 SET `$drugcolumn`='$drugresult' WHERE labno=$labno") or die("ERROR : " . mysqli_error($dbconnection));

		if($drugresult=='Contaminated'){
mysqli_query($dbconnection,"UPDATE results_dst2 SET contdate='$resdate' WHERE labno=$labno") or die("ERROR : " . mysqli_error($dbconnection));
}}

// One update for all other variables
mysqli_query($dbconnection,"UPDATE results_dst2 SET method='$method',resdate='$resdate',comment='$comment',restech='$tech',entrant='$entrant',
entrytime='$entrytime' WHERE labno=$labno") or die("ERROR : " . mysqli_error($dbconnection));

$nextrows=mysqli_query($dbconnection,"SELECT labno FROM results_dst2 WHERE labno>$labno AND resdate='0000-00-00'ORDER BY labno LIMIT 1");
...
<div class="col-md-7">
<h3>RESULTS ENTRY - DST 2ND  LINE</h3>
<?php
...
location.href='resultsentry_dst2.php'
</script> ";
}
...
<fieldset class="scheduler-border"><legend  class="scheduler-border" ><b>DST 2ND LINE RESULTS ENTRY- SAMPLE SEARCH</b></legend>
...
<div class="col-md-3">
<input type="button"value="<<PREVIOUS" class="form-control btn btn-success" onclick="location.href='resultsentry_dst2.php?findlabno=<?php echo @$prevlabno;?>'">
</div>
<div class="col-md-2">
<input type="button" value="NEXT>>" class="form-control btn-info" onclick="location.href='resultsentry_dst2.php?findlabno=<?php echo @$nextlabno;?>'">
</div>
</div>
</div></form>
</fieldset>
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" name="resultsentry-dst2" id="example-1-form" onsubmit="leave()" autocomplete='off'>
...
</div><input type="hidden" name="labno" value='<?php echo $labno; ?>' class="form-control" placeholder="" />
...
?>	
    ...
    <div class="col-sm-8"> 
		...
		<?php
		include("../includes/dbconfig.php");
		$sql="SELECT * FROM resultsoptions_dst2";
...
<div class="form-group">  
	...
	<div class="col-sm-8">
		...
		<?php
		include("../includes/dbconfig.php");
		$sql="SELECT * FROM option_dstmethods WHERE category='dst2'";
		$options=mysqli_query($dbconnection,$sql) or die("ERROR : " . mysqli_error($dbconnection ));
		while ($option = mysqli_fetch_array($options,MYSQLI_ASSOC)) {
			$id = $option['id'];
			$code = $option['code'];
...
<div class="form-group">  
	<div class="col-sm-4"> 
		<input type="submit" name='saveresultsdst2' class="btn btn-success" value='SAVE' class="form-control" placeholder=""/>
    </div>
	<div class="col-sm-8">
...
echo "<script>
alert('Lab No $labno is not registered in DST 2nd Line.');
</script> ";
}
...
<?php
include("../includes/dbconfig.php");
$sql="SELECT * FROM results_dst2 WHERE restech=''";
...
<h4><b>PENDING DST 2 RESULTS</b></h4>

This is a list of pending DST 2  results. [TOTAL = <?php echo $totalempty;?>]<br>
...
?>
<div id="" style='font-size:0.9em;  border:; max-height:100px; width:; margin-right:; padding:; overflow:scroll;'>
<h4><b>QUESTIONED  DST2  RESULTS</b></h4>
This is a list of Questioned  DST 2 results. [TOTAL = <?php echo $totalquestioned;?>]<br>
...
include("../includes/dbconfig.php");
//echo $labno;
$sql="SELECT * FROM results_dst2_hist  where labno=$labno  ";
