...
$nextrows=mysqli_query($dbconnection,"SELECT * FROM results_others WHERE id>$id AND result='' ORDER BY id LIMIT 1");
...
location.href='resultsentry_others.php'
</script> ";
}
...
?>
<div class="col-md-7">
<fieldset class="scheduler-border"><legend  class="scheduler-border" ><b>OTHER TESTS  RESULTS ENTRY- SAMPLE SEARCH</b></legend>
...
<div class="col-md-3">
<input type="button" value="<<PREVIOUS" class="form-control btn-success" onclick="location.href='resultsentry_others.php?findlabno=<?php echo @$prevlabno;?>'">
</div>
<div class="col-md-2">
<input type="button" value="NEXT>>" class="form-control btn-info" onclick="location.href='resultsentry_others.php?findlabno=<?php echo @$nextlabno;?>'">
</div>
</div>
</div></form>
</fieldset>
...
<input type="hidden" name="id" value="<?php echo $id;?>" />
	...
	<select name="resexam"  class="form-control">
			...
			/*
			include("../includes/dbconfig.php");
			$sql="SELECT * FROM option_examination_others ORDER BY name";
			$options=mysqli_query($dbconnection,$sql) or die("ERROR : " . mysqli_error($dbconnection));

			while ($exam = mysqli_fetch_array($options,MYSQLI_ASSOC)) {
				$otherexam = $exam['name'];
				$code= $exam['code'];
				
			echo "<option value='$code' style='background-color:#CCCCCC;'>$otherexam</option>";	
			} */			
			?>
	</select>
</div>
