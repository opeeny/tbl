...
$sql = "UPDATE results_bloodculture SET reviewer='$reviewer',rejreason='$rejreason',reviewdate='$reviewdate',status='Rejected'  WHERE id='$id'";
...
mysqli_query($dbconnection,$sql) or die("ERROR : " . mysqli_error($dbconnection));
$nextrows=mysqli_query($dbconnection,"SELECT * FROM results_bloodculture WHERE id>$id and restech!='' and reviewer=''  ORDER BY id LIMIT 1");
...
$reviewer=$_SESSION['name'];
$reviewdate=date('Y-m-d', time());
$sql = "UPDATE results_bloodculture SET reviewer='$reviewer',reviewdate='$reviewdate', status='Accepted'  WHERE id='$id'";
mysqli_query($dbconnection,$sql) or die("ERROR : " . mysqli_error($dbconnection));
...
location.href='resultsreview_bloodculture.php'
</script> ";
}
if(isset($_GET['media'])){
@$media=$_GET['media'];
...
<legend  class="scheduler-border" ><b>BLOOD CULTURE  RESULTS REVIEW- SAMPLE SEARCH</b></legend>
...
<div class="col-md-3">
<input type="button" value="<<PREVIOUS" class="form-control btn-success" onclick="location.href='resultsreview_bloodculture.php?findlabno=<?php echo @$prevlabno;?>&&media=<?php echo @$prevmedia;?>'">
</div>
<div class="col-md-2">
<input type="button" value="NEXT>>" class="form-control btn-info" onclick="location.href='resultsreview_bloodculture.php?findlabno=<?php echo @$nextlabno;?>&&media=<?php echo @$nextmedia;?>'">
</div>
</div>
</div></form>
</fieldset>
...
<?php }?>
		 ...
		 <div class="col-sm-3"> 
<input type="submit" class="btn btn-success" name="ressubmit_bloodculture" <?php if($status=='Accepted'){?>value="RESULT PASSED REVIEW"<?php }
...
<?php
include("../includes/dbconfig.php");
$sql="SELECT * FROM results_bloodculture WHERE restech!='' and reviewer='' order by labno asc";
...
<b>PENDING BLOOD CULTURE RESULTS REVIEW</b><br>
This is a list of pending BLOOD CULTURE results Review. [TOTAL = <?php echo $totalempty;?>]<br>
...
<b>QUESTIONED  BLOOD CULTURE RESULTS</b><br>
This is a list of Questioned Blood Culture Results Review. [TOTAL = <?php echo $totalquestioned;?>]<br>
