...
$sql = "UPDATE results_dst1 SET reviewer='$reviewer',rejreason='$rejreason',reviewdate='$reviewdate', status='Rejected' WHERE labno=$reslabno";
...
if(isset($_POST['ressubmit_dst1'])){
	$reslabno=mysqli_real_escape_string($dbconnection,$_POST['labno']);
	$entrant=mysqli_real_escape_string($dbconnection,$_POST['entrant']);
$reviewer=$_SESSION['name'];
$reviewdate=date('Y-m-d', time());

$sql = "UPDATE results_dst1 SET reviewer='$reviewer',status='Accepted',reviewdate='$reviewdate' WHERE labno=$reslabno";
mysqli_query($dbconnection,$sql) or die("ERROR : " . mysqli_error($dbconnection));

$nextrows=mysqli_query($dbconnection,"SELECT labno FROM results_dst1 WHERE labno>$reslabno AND result!=''AND status='' ORDER BY labno LIMIT 1");
while ($nextrow = mysqli_fetch_array($nextrows,MYSQLI_ASSOC)) {
$newlabno = $nextrow['labno'];
}
...
<div class="col-md-7">
<h3>RESULTS REVIEW - DST 1st LINE</h3>
<?php
...
location.href='resultsreview_dst1.php'
</script> ";
}
...
<fieldset class="scheduler-border"><legend  class="scheduler-border" ><b>DST 1 RESULTS REVIEW - SAMPLE SEARCH</b></legend>
...
<div class="col-md-3">
<input type="button"value="<<PREVIOUS" class="form-control btn btn-success" onclick="location.href='resultsreview_dst1.php?findlabno=<?php echo @$prevlabno;?>'">
</div>
<div class="col-md-2">
<input type="button" value="NEXT>>" class="form-control btn-info" onclick="location.href='resultsreview_dst1.php?findlabno=<?php echo @$nextlabno;?>'">
</div>
</div>
</div></form>
</fieldset>
...
<?php }?>
		 ...
		 <div class="col-sm-3"> 
<input type="submit" class="btn btn-success" name="ressubmit_dst1" <?php if($status=='Accepted'){?>value="RESULT PASSED REVIEW"<?php }
...
<?php
include("../includes/dbconfig.php");
$sql="SELECT * FROM results_dst1 WHERE reviewer=''";
