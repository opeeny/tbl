...
<div class="col-md-10">
<h3 class="red">To view All Samples TAT , Simply Click on test on left side menu,then Download Button</h3>
<div class="row">
...
$select_tat="SELECT * FROM  option_examination_others where code='$code'";
	...
	while ($tat = mysqli_fetch_array($tats,MYSQLI_ASSOC)) {
		...
		where s.labno=f.labno  and f.test='$code'";
	
}else{
		
$sql="SELECT s.*,f.* from samples s,results_others f 
where s.labno=f.labno  
and  (s.rctdate between '".$fromdate."' and '".$todate."') and f.test='$code'";
}
}
if($code=='dst2'){ 
$title='DST 2  SAMPLES TAT';
  $testentry='dst2';
$code=$code;
...
$sql="SELECT s.*,f.* from samples s,results_dst2 f where s.labno=f.labno 
 and  (s.rctdate between '".$fromdate."' and '".$todate."')";
}
}
if($code=='dst1'){ 
$title='DST 1  SAMPLES TAT';
  $testentry='dst1';
$code=$code;
$select_tat="SELECT * FROM  option_examination where code='$code'";
	$tats=mysqli_query($dbconnection,$select_tat) or die("ERROR : " . mysqli_error($dbconnection));
...
$title='BLOOD CULTURE SAMPLES TAT';
...
$sql="SELECT s.*,f.* from samples s,results_bloodculture f 
where s.labno=f.labno and  (s.rctdate between '".$fromdate."' and '".$todate."')";
}
if($code=='solidculture'){ 
$title='SOLID CULTURE SAMPLES TAT';
...
$title='LIQUID CULTURE SAMPLES TAT';
...
if($code=='fm'){ 
$title='MICROSCOPY FM SAMPLES TAT';
$testentry='fm'; 
$code=$code;
$select_tat="SELECT * FROM  option_examination where code='$code' ";
	$tats=mysqli_query($dbconnection,$select_tat) or die("ERROR : " . mysqli_error($dbconnection));
...
if ($fromdate=='' or $todate==''){
$sql="SELECT s.*,f.* from samples s,results_fm f where s.labno=f.labno";
} else{
...
if($code=='zn'){ 
$title='MICROSCOPY ZN SAMPLES TAT';
 $testentry='zn';
$code=$code;
$select_tat="SELECT * FROM  option_examination where code='$code'";
	$tats=mysqli_query($dbconnection,$select_tat) or die("ERROR : " . mysqli_error($dbconnection));
...
?><br>
<button class="btn btn-success" onclick="location.href='download_overduesamples.php?action=<?php echo $code?>&&fromdate=<?php echo $fromdate ?>&&todate=<?php echo $todate ?>'">Download</button>SAMPLES TAT
...
$media=$sample['media'];
$resdate=$sample['resdate'];
if($resdate=='0000-00-00'){$readresdate='';}
$rctdatetime=$rctdate.' '.$rcttime;
$today=date('Y-m-d H:i:s', time());
$date1=date_create("$rctdate");


$date1=date_create("$accessiontime");
$date2=date_create("$today");
if($resdate=='0000-00-00'){$date2=date_create("$today");}
else{$date2=date_create("$resdate"); }
