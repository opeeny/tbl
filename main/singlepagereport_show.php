...
</div><div class="lefcontact2 contact"> LAB No: <?php 
if($studycode=='BDM'){echo $labno;}else {echo $studycode .'-'. $labno; }?>
</div>
<br><p>
<b>PATIENT DETAILS</b>
...
<tr valign='top'>
	<td>
	Patient ID:&emsp;<b><?php echo $pid;?></b></td>
...
<tr valign='top'>
	...
	<td>
	<b>Date Collected</b>:&emsp;<?php if($colldate=='0000-00-00'){echo '';} else echo @date('d-M-y',strtotime($colldate));?>&emsp;<?php if($colltime!='00:00:00'){echo date('h:i A',strtotime($colltime));}?><br>
	<b>Date Received</b>:&emsp;<?php if($rctdate=='0000-00-00'){echo '';} else echo @date('d-M-y',strtotime($rctdate));?>&emsp;<?php if($rcttime!='00:00:00'){echo date('h:i A',strtotime($rcttime));}?><br>
	<b>Processed</b>:&emsp;<?php if($processdate=='0000-00-00'){echo '';} else echo @date('d-M-y',strtotime($processdate));?><br>
...
<table class="table table-condensed">
<tr class="boldhead"><td>Examination</td><td>Result</td><td>Date</td><td>Tech</td></tr>
<?php if($fmcount>0){?>
...
<td><?php if(@$dfmreviewstatus){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>";}?></td>
</tr>
<?php 
}
...
<td><?php if(@$dznreviewstatus){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>";}?></td>
...
<td><?php if(@$gnxreviewstatus){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>";}?> </td><?php
...
<table class="table table-condensed">
<tr class="boldhead"><td>Media</td><td>Result</td>
<?php if($showliquid=='Show'){ ?><td>Days To Positive</td><?php } ?><td>Date</td><?php if($showtech=='Show'){ ?><td>Tech</td><?php } ?></tr>
	...
	<td><?php echo $liquidculture_resdtp;?></td> <?php } ?>
	
	<td><?php echo $liquidculture_resdate?> 
	</td>
	<?php if($showtech=='Show'){ ?><td><?php echo $dtp_tech;?></td> <?php }?>
	<td><?php if($liquidreviewstatus=="notreviewed"){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>"; }?>
	</td></tr>
<?php 
}
echo "</table>";
...
<table class="table table-condensed">
<tr class="boldhead"><td>Media</td><td>Result</td>
<?php if($showsolid=='Show'){ ?><td>Semi Quantitative</td> <?php } ?><td>Date</td>
...
while ($culture_interpretation = mysqli_fetch_array($culture_interpretations,MYSQLI_ASSOC)) {
		...
		<td> <?php if($solidreviewstatus=="notreviewed"){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>"; }?></td>
		</tr>
<?php 
}
echo "</table>";
...
<table class="table table-condensed">
<tr class="boldhead"><td>Media</td><td>Result</td><td>Date</td></tr>
...
while ($bloodculture= mysqli_fetch_array($bloodcultures,MYSQLI_ASSOC)) {
		...
		<td><?php echo $bloodculture_resdate?> <?php if($bloodreviewstatus=="notreviewed"){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>"; }?></td>
		</tr>
<?php 
}
echo "</table>";
...
<?php if($dst1count2>0){?>	
		...
		<?php if($dst1reviewstatus=="notreviewed"){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>"; }?><br>
...
<?php if($dst2count2>0){?>	
		...
		<?php if($dst2reviewstatus=="notreviewed"){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>"; }?><br>
...
<?php
	...
	<tr>
		...
		<td><?php echo $othertestresdate?> <?php if($othertestreviewstatus=="notreviewed" and $othertestresult!==''){ echo "<font color='red'>  >>Results Entry Pending Reviewed</font>"; }?></td>
		</tr>
<?php 
}
echo "</table>";
